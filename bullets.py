...
class Bullet:
    bullet_image = None
    bullet_hit_image = None
    def __init__(self , x , y):
        if (self.bullet_image == None):
            self.bullet_image = load_image('Tutorial/Tutorial_bullet.png')
        if (self.bullet_hit_image == None):
            self.bullet_hit_image = load_image('Tutorial/Tutorial_bullet_BAAM.png')
    ...
    def draw(self):
        if self.state == 0:
            self.bullet_image.clip_composite_draw(0, 0, 29, 30, 0 + self.ratate, '', self.x, self.y, 29, 30)
        elif self.state == 1:
            self.bullet_hit_image.clip_draw(0, 0, 51, 45, self.x, self.y, 29 + self.action, 30 + self.action)

    def update(self):
        if self.state == 0:
            self.x += self.velocity * game_framework.frame_time  # 총알 속도
            self.ratate += 0.2  # 총알의 회전
        elif self.state == 1: # 총알 상태
